...
# todo: update model
interpreter = tf.lite.Interpreter(model_path="./data/model_final.tflite")
...
def init_participant(gender, age_range):
    ...
    participant_data = {"gender": gender, "age": age_range}
...
def find_word_in_audio(audio, sample_rate=SAMPLE_RATE, word_duration=1):
    ...
    amplitudes = [
        np.mean(np.abs(audio[i : i + window_size]))
...
def test():
    ...
    return render_template(
        "test.html", title=TITLE, participant_id=participant_id, experiments=EXPERIMENTS
    )
...
def results():
    ...
    noise_level = get_background_noise_level(
        os.path.join(participant_dir, "noise_0.wav")
    )
